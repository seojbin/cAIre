...
def extractfeatures(trajectories):
    ...
    for traj in trajectories:
        ...
        structural_sigma = 5.0  # 데이터 길이에 따라 조절 (보통 5~10 추천)
        traj_structural = gaussian_filter1d(traj, sigma=structural_sigma, axis=0)

        diffs_struct = np.diff(traj_structural, axis=0)
        length_struct = np.sum(np.linalg.norm(diffs_struct, axis=1))
        efficiency = 1.0
        if max_reach > 1e-3:
            efficiency = length_struct / (2 * max_reach)
