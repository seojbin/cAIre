    ...
    def _resolve_indices(self, config_item):
        ...
        for item in config_item:
            if isinstance(item, str):
                try:
                    indices.append(self.feature_names_list.index(item))
                except ValueError:
                    print(f"Warning: Feature name '{item}' not found in extractor output. Ignoring.")
            else:
                indices.append(item)
        return indices
...
def main():
    ...
    mode_str = str(NOISE_MODE)
    print(f"\nRobustness Test: {mode_str}")
    print(f" -> Levels: {STRESS_LEVELS}")
    print(f" -> Extractor Module: {EXTRACTOR_MODULE_NAME}")
                    ...
                    for idx in val_real_indices:
                        ...
                        if np.any(preds != y_val):
                            ...
                            if model_name != "Unknown":
                                if model_name == "M1": m_indices = model.select_indices_model1
                                elif model_name == "M2": m_indices = model.select_indices_model2
                                elif model_name == "M3": m_indices = model.select_indices_model3
                                elif model_name == "M4": m_indices = model.select_indices_model4
                                failed_model_features = [all_feature_names[fi] for fi in m_indices]
                            ...
                            error_logs.append({
                                ...
                                'Config': conf_name,
                                'Kernel': kernel_type,
                                'Extractor': EXTRACTOR_MODULE_NAME,
                                'Mode': str(NOISE_MODE),
                                'Level': level,
                                'Sample_ID': idx,
                                'Label': inv_label[true_label_code],
                                'Predicted': inv_label[pred_label_code],
                                'Cause': cause,
    ...
    txt_filename = f"robustreport_{timestamp}.txt"
    csv_filename = f"robust_error_details_{timestamp}.csv"

    with open(txt_filename, "w", encoding="utf-8") as f:
        f.write("Robustness Report\n")
        f.write(f"Date: {timestamp}\n")
        f.write(f"Extractor Module Used: {EXTRACTOR_MODULE_NAME}.py\n")
        f.write("=================\n\n")

        f.write("[Feature Definitions by Configuration]\n")
        for c_name, c_cfg in FEATURE_CONFIGS.items():
            f.write(f"Configuration: {c_name}\n")
            for m_key, m_val in c_cfg.items():
                if len(m_val) > 0 and isinstance(m_val[0], int):
                    m_feat_names = [all_feature_names[i] for i in m_val]
                else:
                    m_feat_names = m_val
        ...
        if not error_logs:
            ...
            f.write("[Top Failure Causes by Configuration]\n")
            for conf in df['Config'].unique():
                f.write(f"\n>> Configuration: {conf}\n")
                subset = df[df['Config'] == conf]
                f.write(subset['Cause'].value_counts().head(10).to_string())
                f.write("\n")
            f.write("\n")

            f.write("[Detailed Error Log Sample (Top 50)]\n")
            cols_to_show = ['Config', 'Extractor', 'Level', 'Label', 'Predicted', 'Cause', 'Failed_Model']
            f.write(df[cols_to_show].head(50).to_string())
            f.write(f"\n\n* Full details are saved in '{csv_filename}'")

            df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
            print(f"\nTotal failure records: {len(df)}")
            print(f"Logs saved to: {txt_filename}, {csv_filename}")
