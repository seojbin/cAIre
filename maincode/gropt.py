...
def evaluate_robustness(model_type, feature_indices, x_raw, y_raw, stress_n):
    ...
    # Model 1: Horizontal(0) vs Others(1)
    if model_type == 1:
        x_curr = x_raw
        # 가로는 0, 나머지는 1
        y_curr = np.where(y_raw == label['horizontal'], 0, 1)

    # Model 2: Vertical(0) vs Complex(1) (Horizontal 제외)
    elif model_type == 2:
    ...
    # Model 3: Circle(0) vs Diagonals(1) (H, V 제외)
    elif model_type == 3:
    ...
    # Model 4: Left(0) vs Right(1) (Diagonal Only)
    elif model_type == 4:
...
def find_best_robust_features(model_id, x_raw, y_raw, feature_names):
    ...
    for attempt in range(MAX_STRESS_ATTEMPTS + 1):
        ...
        if attempt < MAX_STRESS_ATTEMPTS:
            print(f"    -> Perfect Score. Increasing noise (+{STRESS_STEP})...")
            current_stress += STRESS_STEP
        else:
            print("    -> Max stress reached.")
